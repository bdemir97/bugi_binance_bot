...
from indicators.indicators import volatility, rsi, heikin_ashi, moving_averages, dema
...
def sell_decision(symbol1, symbol2, binance_spot_api, last_price, wallet):
    ...
    CANDLE_LENGTH = config_manager.get("CANDLE_LENGTH")
    ...
    price_change_percentage = volatility(symbol, binance_spot_api, CANDLE_LENGTH, config_manager.get("PRICE_CHANGE_CALCULATION"))
    
    if price_change_percentage <= (config_manager.get("VOLATILITY_THRESHOLD")*-1):
        logging.info(f'Decided to sell based on high volatility (Price change: {round(price_change_percentage,2)}%)')
    ...
    if DECISION_ALGORITHM == 1:
        if rsi(symbol, config_manager.get("RSI_DURATION"), binance_spot_api, CANDLE_LENGTH) >= config_manager.get("RSI_THRESHOLD_SELL"):
            if heikin_ashi(symbol, binance_spot_api, CANDLE_LENGTH,config_manager.get("HEIKIN_DURATION")) < 0:
        ...
        if rsi(symbol, config_manager.get("RSI_DURATION"), binance_spot_api, CANDLE_LENGTH) >= config_manager.get("RSI_THRESHOLD_SELL"):
            if heikin_ashi(symbol, binance_spot_api, CANDLE_LENGTH,config_manager.get("HEIKIN_DURATION")) < 0:
                ma_short, ma_long = moving_averages(symbol, binance_spot_api, CANDLE_LENGTH, config_manager.get("SHORT_MA"), config_manager.get("LONG_MA"))
                if ma_short < ma_long:
        ...
        dema_short, dema_long = dema(symbol, binance_spot_api, CANDLE_LENGTH, config_manager.get("SHORT_DEMA"), config_manager.get("LONG_DEMA"))
...
def buy_decision(symbol1, symbol2, binance_spot_api, last_price, wallet):
    ...
    if price_change_percentage >= config_manager.get("VOLATILITY_THRESHOLD"):
        logging.info(f'Decided to buy based on high volatility (Price change: {round(price_change_percentage,2)}%)')
        return buy(binance_spot_api, symbol1, symbol2, wallet, initial1, initial2)
    
    DECISION_ALGORITHM = config_manager.get("DECISION_ALGORITHM")
    if DECISION_ALGORITHM == 1:
        if rsi(symbol, config_manager.get("RSI_DURATION"), binance_spot_api, CANDLE_LENGTH) <= config_manager.get("RSI_THRESHOLD_BUY"):
            if heikin_ashi(symbol, binance_spot_api, CANDLE_LENGTH,config_manager.get("HEIKIN_DURATION")) > 0:
